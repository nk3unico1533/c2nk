...
io.on('connection', (socket) => {
        ...
        if (data.type === 'agent') {
            ...
            if (existing) {
                ...
                io.to('ui_room').emit('agent_event', { 
                    ...
                    type: 'SYSTEM', agentId: newAgent.id, timestamp: Date.now(), payload: 'Node Joined 1533 Swarm' 
                });
            }
            io.to('ui_room').emit('agents_list', agents);
        }
    ...
    socket.on('exec_cmd', (data) => {
        ...
        io.to('ui_room').emit('agent_event', { id: Date.now().toString(), timestamp: Date.now(), type: 'SYSTEM', agentId: 'SERVER', payload: `Exec: ${cmd}` });
